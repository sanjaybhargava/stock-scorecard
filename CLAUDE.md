...
|------------|---------|-------------|
| `import` | Parse tradebooks, FIFO match, fetch dividends, auto-run cockpit, auto-serve UI in browser | Optional (`--wizard`) |
...
│   │   ├── import.go            # "import" — parse raw CSVs, FIFO match, dividends, F&O, auto-serve
...
│   │   ├── view.go              # Embedded web server for scorecard + cockpit UIs
│   │   └── embed/               # Pre-built UIs embedded in binary
│   │       ├── dist/            # Scorecard React UI build
│   │       └── cockpit-dist/    # Cockpit React UI build
│   └── serve/
...
├── deploy-friend.sh             # Deploy a friend's scorecard + cockpit to gh-pages
├── CLAUDE.md
├── go.mod
└── go.sum
```
...
```bash
# Parse tradebooks, FIFO match, fetch dividends, F&O attribution, score, auto-serve UI
...
```
Parse EQ/FnO CSVs → FIFO Match (Tier 1) → Transfer-In Detection (Tier 2) → Dividends → F&O Attribution → Score → JSON → Auto-Cockpit → Serve UI
```
...
- **Flexible CSV headers:** EQ/FO parsers use column-name lookup (not positional), survives Zerodha format changes (extra columns, reordering, spacing)
- **EQ vs FO detection:** EQ parser rejects files with `expiry_date` column to avoid parsing F&O trades as equity
- **Client ID formats:** Regex supports 2-3 letter prefixes (e.g. `BT2632`, `DUA527`); `--client` flag overrides auto-detection
- **Filename conventions:** Supports both `{clientID}_dates.csv` and `{clientID}_EQ_dates.csv` / `{clientID}_FO_dates.csv`

## Self-Contained Binary

The `import` command embeds both the scorecard and cockpit React UIs. After processing trades, it:

1. Generates scorecard JSON (realized trades)
2. Auto-runs cockpit with defaults (unrealized portfolio, fetches Yahoo Finance prices)
3. Starts a local web server and opens two browser tabs:
   - **Realised scorecard:** `http://127.0.0.1:{port}/`
   - **Unrealised cockpit:** `http://127.0.0.1:{port}/cockpit/?client={clientID}`

No Node.js, no GitHub, no setup needed. The friend just runs:

```bash
./stock-scorecard-mac-m import --client THEIRID
```

### Building Embedded Binaries

```bash
# 1. Build UIs for embedding (relative base paths)
cd ui && npx vite build --base "./" --outDir ../cmd/scorecard/embed/dist
cd ui-cockpit && npx vite build --base "./" --outDir ../cmd/scorecard/embed/cockpit-dist

# 2. Cross-compile binaries (UIs are embedded via //go:embed)
./build-release.sh    # → ./dist/stock-scorecard-mac-m, -mac-intel, -windows.exe
```
...
# Deploy a friend's scorecard + cockpit to GitHub Pages
# (expects scorecard_<ID>.json and cockpit_<ID>.json in ~/Downloads)
...
- Scorecard (per client): https://sanjaybhargava.github.io/stock-scorecard/?client=DUA527
- MIL cockpit: https://sanjaybhargava.github.io/vimal-stock-scorecard/cockpit/?client=ZY7393
- Cockpit: https://sanjaybhargava.github.io/stock-cockpit/?client=DUA527
...
- ~~Self-contained binary with embedded UI~~ ✓ Implemented (view.go, embed/)
- ~~Flexible CSV parser for Zerodha format changes~~ ✓ Implemented (column-name lookup)
- ~~Support 3-letter client IDs (e.g. DUA527)~~ ✓ Implemented
